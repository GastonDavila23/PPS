...
# Importamos tu procesador original
...
def get_asignaciones():
    ...
    # REUTILIZACIÓN: Usamos el mismo constructor SQL para ver y para descargar
...
def cargar():
    if 'planillas' not in request.files: return jsonify({"error": "Faltan archivos"}), 400
    files = request.files.getlist('planillas')
    
    conn = get_db_connection()
    try:
        df_old = pd.read_sql_query("SELECT * FROM escuelas_data", conn)
    except:
        df_old = pd.DataFrame()
    finally:
        conn.close()

    # Tu procesador original
    df_new = procesar_archivos_y_actualizar(files, df_old)
    ...
    return jsonify({"error": "Fallo el procesamiento"}), 400
...
def descargar():
    filtros = {
        ...
        'turno': request.args.get('turno'),
        'estado': request.args.get('estado_asignacion')
        # 'nombre': request.args.get('nombre_escuela') # Opcional si quieres filtrar excel por nombre también
    }
